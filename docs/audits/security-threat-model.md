...
Encore is a single-tenant, self-hosted service. The current F0 surface is the
FastAPI/CLI process, SQLite database, adjacent Fernet key, container image, and
health endpoints. The highest-value current asset is the encrypted Plex token;
future F1-F5 assets include taste-rich artist/play data, Apprise destinations,
and RSS/iCal feed tokens.
