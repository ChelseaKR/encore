...
| Taste information reaches an unintended person | **Both egress surfaces now exist.** F4 delivers only to per-channel destinations the user added by hand (never defaulted, never aggregated); F5's feeds are reachable only with an unguessable, encrypted-at-rest, rotatable capability token, and every unauthorized shape is one indistinguishable 404 (`docs/adr/0013`). `no_outing` marker tests are merge-blocking for both | Accepted as RR-04 (push) and RR-06 (pull): a destination or URL the user chose *is* the feature. Residual exposure is the token in the operator's own access logs; per-subscriber revocation is unbuilt (RR-07) |
