...
| CodeQL | AUTO (SEC-08) | 0 alerts | 5 | Python + Actions packs trigger on every `main` update, weekly, and on dispatch. `codeql.yml` sets `upload: always` — code scanning is free on a public repository, and the earlier `upload: never` (written when this repo was private) was discarding every analysis: the SARIF was created on the runner and deleted with it, so no alert, no history and no dismissal record survived a run. Uploading did **not** replace the gate: `codeql.yml` still fails the run on any finding, because an alert in a tab does not stop a merge. The account-budget block that once prevented these jobs from starting (roadmap B1/U6) is no longer in force — runs on `main` are green |
| Secret scan (gitleaks) | AUTO (SEC-17/18) | clean | 5 | Met (pre-commit + CI) |
| Scorecard aggregate | AUTO (SEC-37) | ≥8 | 5 | Not yet run — and no longer blocked by visibility. The public-repo precondition it was deferred on is met (ADR 0010's 2026-08-29 correction). What is missing is the workflow: no Scorecard job exists in `.github/workflows/`. Wiring one requires deciding whether results are published to the OpenSSF API or kept as code-scanning alerts, which is a publication decision (issue #52) rather than a mechanical one |
...
| CodeQL `actions` pack | AUTO (CICD-20) | 0 alerts | 5 | Wired 2026-07-05 (`codeql.yml`); automatic triggers restored 2026-07-14; same matrix, same upload and same in-run gate as the CodeQL row above. Its contexts are not required on a pull request because `codeql.yml` has no `pull_request` trigger — recorded in ADR 0010's correction and pinned by `tests/test_branch_protection.py` |
...
| MB rate-limit violations (soak counter) | AUTO (project) | 0 | 4 | Polling code exists (F3, 2026-07-31): all MB traffic — search and browse — shares one process-global limiter, tested at the unit level. **Amended 2026-09-06 (#63):** that limiter is still one per process, but its interval is now resolved from the configured endpoint — pinned at 1 req/s against MetaBrainz's own host whatever `ENCORE_MB_RATE_LIMIT` says, and operator-set only against a self-hosted mirror (`src/encore/endpoints.py`, `docs/self-hosted-mirror.md`). The violations this row counts are violations against the PUBLIC host, so the target is unchanged; the soak *counter* itself still needs a real 24h deployment and lands with the M2 exit soak |
